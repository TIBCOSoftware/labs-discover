...
class SparkScheduledOnK8s() {
  ...
  val k8sSparkKind = "ScheduledSparkApplication"
  ...
  def sparkApplication(pmConfig: pmConfigLiveApps): (String, Int, String, String) = {
    ...
    val templateName: String = DiscoverConfig.sparkScheduledFile.toAbsolutePath.toString
    ...
    Try {
      ...
      case Success(value) => {
        log.info("ok")
        (s"spark-pm-${meta_name} has been sucsesfully put to Orbit", returnCode, "CREATED", s"spark-pm-${meta_name}")
      }
      case Failure(exception: ApiException) => {
        log.error(s"Error : ${exception.getCode} with ${exception.getResponseBody} ")
        (s"spark-pm-${meta_name} K8s Error : ${exception.getResponseBody} ", exception.getCode, "ERROR", s"spark-pm-${meta_name}")
      }
      case Failure(x: Throwable) => {
        log.error(s"Error : ${x.getMessage}")
        (s"spark-pm-${meta_name} Error", 100, "YOUR_LAST_MISTAKE", s"spark-pm-${meta_name}")
      }
    }
  }
