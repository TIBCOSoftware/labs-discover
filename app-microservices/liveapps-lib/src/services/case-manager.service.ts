...
import { DeleteCasesResponse, GetCaseResponseItem, GetTypeResponseItem, CMModelError, PutCasesRequest } from '../model/casemanager.models';

export class CaseManagerService  {

  private basePath = 'https://eu.liveapps.cloud.tibco.com/case/v1'
   ...
   */
  public getCases = async (token: string, sandbox?: string, filter?: string, select?: string, skip?: string, top?: string, search?: string, count?: boolean): Promise<GetCaseResponseItem[]|CMModelError|number> => {
    ...
    return await axios.get( localVarPath, {
   ...
   */
  public deleteCases = async (token: string, sandbox: string, filter: string) : Promise<DeleteCasesResponse|CMModelError> => {
    const localVarPath = this.basePath + '/cases';
    const localVarQueryParameters: any = {};
    ...
    return await axios.delete( localVarPath, {
      headers: { 'Authorization': 'Bearer ' + token },
      params: localVarQueryParameters
    }).then(response => {
      return response.data;
   ...
   */
  public updateCase = async (token: string, caseReference: string, sandbox: string, select: string, payload: PutCasesRequest) : Promise<GetCaseResponseItem|CMModelError> => {
    const localVarPath = this.basePath + '/cases/{caseReference}'.replace('{' + 'caseReference' + '}', encodeURIComponent(String(caseReference)));
    const localVarQueryParameters: any = {};
    ...
    return await axios.put( localVarPath, payload, {
      headers: { 'Authorization': 'Bearer ' + token },
      params: localVarQueryParameters
    }).then(response => {
      return response.data;
   ...
   * @param user Test userId for Development sandbox only. Claims must still contain a valid userid but will be overridden when the optional $user parameter is specified    
   */
  public getCase = async (token: string, caseReference: string, sandbox: string, select?: string): Promise<GetCaseResponseItem[]|CMModelError> => {
    const localVarPath = this.basePath + '/cases/{caseReference}'.replace('{' + 'caseReference' + '}', encodeURIComponent(String(caseReference)));
    const localVarQueryParameters: any = {};
   ...
   */
  public getTypes = async (token: string, sandbox: string, select?: string, filter?: string, skip?: string, top?: string, count?: boolean): Promise<GetTypeResponseItem[]|CMModelError> => {
