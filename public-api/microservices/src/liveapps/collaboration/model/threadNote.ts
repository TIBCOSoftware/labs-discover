...
/**
* Used to create a top-level Note with optional Roles applied to the Thread created for this Note.  Although Roles can be applied to a Thread at any time, care should be taken to apply Roles at the time a top-level Note is created if there is a need to restrict who can view the Note content or receive Collection or Topic Notifications if they exist. If Notifications are currently set on the Topic or a Collection associated with the Topic, those notifications are sent based on initial Roles set with the top-level Note when created.  If you wait to set the Role at a later time, a User may view the Note or receive a Notification before you submit the request to restrict access.
*/
export class ThreadNote {
    /**
    * The type of the Related Item. - This must be a non-zero length value (limited to 100 characters in length) and must be specified together with relatedItemId. - Any UTF-8 characters can be used. - The property is treated as case sensitive and white space is significant (not ignored). - All relatedItemType values that start with \'LIVEAPPS_\', \'DT_\', or \'RT_\' are reserved for internal use by Live Apps. 
    */
    'relatedItemType': string;
    /**
    * The ID for the Related Item. - This must be a non-zero length value (limited to 100 characters in length) and must be specified together with relatedItemType. - Any UTF-8 characters can be used. - The property is treated as case sensitive and white space is significant (not ignored). 
    */
    'relatedItemId': string;
    /**
    * An optional list of Collection name values used to associate a new Topic specified by relatedItemType and relatedItemId to the given Collection names, default = null. - Each name value must be a non-zero length value (limited to 100 characters in length). - Any UTF-8 characters can be used. - The name values are treated as case sensitive and white space is significant (not ignored).  The relatedItemCollection value is applied to new Topics only and is ignored if the Topic already exists.  To modify the relatedItemCollection value of an existing topic use the PUT /topics request. 
    */
    'relatedItemCollection'?: Array<string>;
    'note': Note;
    /**
    * The Roles applied to the Thread initiated by this top-level Note.
    */
    'roles'?: Array<ThreadRole>;
    /**
    * The Notifications applied to the Thread initiated by this top-level Note.
    */
    ...
    */
    ...
    static getAttributeTypeMap() {
        return ThreadNote.attributeTypeMap;
    }
}
