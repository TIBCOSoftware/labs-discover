...
import { ActionPerformedFilesUrl } from '../model/actionPerformedFilesUrl';
...
export class FilesOperationsApi {
    ...
    public async getRouteFileContent (filename: string, orgId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ActionPerformedFilesUrl;  }> {
        const localVarPath = this.basePath + '/files/download/signed/{orgId}/{filename}'
            .replace('{' + 'filename' + '}', encodeURIComponent(String(filename)))
            .replace('{' + 'orgId' + '}', encodeURIComponent(String(orgId)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ActionPerformedFilesUrl;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ActionPerformedFilesUrl");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
