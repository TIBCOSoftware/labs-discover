...
/**
* Assigns a Thread access Role to an Organization Model Entity.  Provides access to Notes associated with a Thread to which the Role is assigned.  Each ThreadRole matches a Role type (OWNER, MODERATOR, REPLIER, or VIEWER) to an Organization Model Entity (Group or User).  The current User\'s memberships in the Organization Model then determines which Roles the User has to access a given Thread, or Notes in the Thread.  The User who creates a top-level Note implicitly has a Role of OWNER.  If a Thread or an Organization Model Entity is deleted, any associated ThreadRole is also deleted automatically.
*/
export class ThreadRole {
    /**
    * The unique identifier for an Organization Model Entity (Group or User).
    */
    'entityId': string;
    /**
    * The type of Organization Model Entity specified by entityId.
    */
    'entityType': ThreadRole.EntityTypeEnum;
    /**
    * The Role controlling access to Notes and the Thread.  One of [ \'VIEWER\', \'REPLIER\', \'MODERATOR\', \'OWNER\', \'THREAD_DELETER\', \'THREAD_VIEWER\', \'COLLECTION_MODERATOR\' ].   __Cumulative Roles:__   The following four Roles are cumulative, with each successive Role adding more access to what is granted by the previous Role.   - A VIEWER can: View Notes or the Thread, receive Notifications, add / modify / remove Notifications for themselves on a Collection, Topic, or Thread.   - A REPLIER can also: Add a reply Note.   - A MODERATOR can also: Lock or unlock the Thread, hide or unhide a Note, accept or unaccept a reply, add / modify / remove Thread Notifications and Roles.   - An OWNER can also: Edit Note Attributes, Delete Notes, Delete Threads (see THREAD_DELETER Role), Update the relatedItemCollection value of a Topic (see COLLECTION_MODERATOR Role)   __Non-Cumulative Roles:__   The following Roles are not cumulative, but grant specific types of access.   - A THREAD_DELETER can:     - Delete Threads       A Thread is deleted when:       - The top-level Note for a Thread is deleted.      - The Topic a Thread belongs to is deleted.      - A Topic is deleted that matches the Related Item that a Thread is lifecycled with.    - A THREAD_VIEWER can:     - View Threads       This allows the User to view the Thread but not the Notes in the Thread.   - A COLLECTION_MODERATOR can:     - Update the relatedItemCollection value of a Topic       This allows the User to update the relatedItemCollection value of a Topic using the PUT /topics request.  The User must have COLLECTION_MODERATOR access to all of the Threads that are under a Topic to perform this request.  
    */
    'role': ThreadRole.RoleEnum;
    /**
    * Set by the service.  The unique identifier for this ThreadRole assigned by the service when created.
    */
    'id'?: string;
    /**
    * Set by the service.  The unique identifier for the Thread that this ThreadRole is assigned to.
    */
    'threadId'?: string;
    /**
    * Set by the service.  The display name of the Organization Model Entity specified by entityId.  This value is set by the service when a ThreadRole is created and is used for display purposes only.
    */
    'entityName'?: string;
    ...
    static attributeTypeMap: Array<{name: string, baseName: string, type: string}> = [
        ...
        },
        {
            "name": "entityType",
            "baseName": "entityType",
            "type": "ThreadRole.EntityTypeEnum"
        },
        {
            "name": "role",
            "baseName": "role",
            "type": "ThreadRole.RoleEnum"
        },
        {
            "name": "id",
            "baseName": "id",
    ...
    static getAttributeTypeMap() {
        return ThreadRole.attributeTypeMap;
    }
}

export namespace ThreadRole {
    ...
    export enum RoleEnum {
        ...
        Replier = <any> 'REPLIER',
        Viewer = <any> 'VIEWER',
        ThreadDeleter = <any> 'THREAD_DELETER',
        ThreadViewer = <any> 'THREAD_VIEWER',
        CollectionModerator = <any> 'COLLECTION_MODERATOR'
    }
}
