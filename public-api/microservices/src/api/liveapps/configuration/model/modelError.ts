...
import { ErrorContextAttribute } from './errorContextAttribute';

/**
* The JSON payload to be returned when either a 400 or 500 HTTP status code is returned.
*/
export class ModelError {
    ...
    */
    'errorMsg'?: string;
    /**
    * The following are the possible error codes in the Shared Client State Service (note that the description shown is not part of the error code): - SS_DB_READ_ERROR - Error while reading an entity. - SS_DB_WRITE_ERROR - Error while writing an entity. - SS_DB_DELETE_ERROR - Error while deleting an entity. - SS_DB_NULL_ENTITY - Attempt to write a null entity. - SS_DB_NO_SUBSCRIPTION - No Subscription can be identified from security context. - SS_DB_MANDATORY_PROPERTY_MISSING - A mandatory property has not been supplied in the request. - SS_DB_NOT_AUTHENTICATED - No authenticated caller can be identified from the security context. - SS_DB_CONSTRAINT_ERROR - A constraint error occurred while writing an entity. - SS_SRVC_UNKNOWN_ENTITY - Unable to locate the identified entity. - SS_SRVC_NOT_AUTHENTICATED - Unable to identify the caller. - SS_SRVC_SERVICE_NOT_AUTHORIZED - The caller is not authorized to access the identified service. - SS_SRVC_ENTITY_NOT_AUTHORIZED - The caller is not authorized to access the identified entity. - SS_SRVC_SANDBOX_NOT_AUTHORIZED - The caller is not authorized to access the identified sandbox. - SS_SRVC_NOT_REGISTERED - The caller is not a registered Shared Client State Service user. - SS_SRVC_MISSING_MANDATORY_PARAMETER - A mandatory parameter has not been supplied in the request. - SS_SRVC_INVALID_PARAMETER - A parameter supplied in the request is invalid. 
    */
    'errorCode'?: string;
    /**
    * Added if available and requested by X-Provide-Stacktrace HTTP header.
    */
    'stackTrace'?: string;
    /**
    * A set of named attributes that may be relevant to the error in order to determine the cause of the error.
    */
    'contextAttributes'?: Array<ErrorContextAttribute>;
    ...
    static attributeTypeMap: Array<{name: string, baseName: string, type: string}> = [
        ...
        },
        {
            ...
            "type": "Array<ErrorContextAttribute>"
        }    ];
